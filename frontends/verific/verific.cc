...
void VerificImporter::import_attributes(dict<RTLIL::IdString, RTLIL::Const> &attributes, DesignObj *obj, Netlist *nl)
{
  ...
  if (nl) {
    ...
    if (type_range->IsTypeScalar()) {
      // printf(" range [%lld:%lld]", tr->GetScalarRangeLeftBound(),tr->GetScalarRangeRightBound());
...
void VerificImporter::import_netlist(RTLIL::Design *design, Netlist *nl, std::map<std::string,Netlist*> &nl_todo, bool norename)
{
  ...
  FOREACH_NET_OF_NETLIST(nl, mi, net)
	...
	log("  importing net %s as %s.\n", net->Name(), log_id(wire_name));
      ...
      net_map[net] = wire;
      // assertOrAssumeRange(net);
      const char *port_name = net->Name();
      const TypeRange *tr = nl->GetTypeRange(port_name) ;
      if (tr) {
	printf("Not a net bus: %s %s",port_name, tr->Dump());
	if (tr->IsTypeScalar()) {
	  printf(" range [%lld:%lld]", tr->GetScalarRangeLeftBound(),tr->GetScalarRangeRightBound());
	}
	printf("\n");
	// const Set *ports = net->GetPorts();
	// printf("Number of ports [%u]", ports->Size());
      }
    }
  ...
  FOREACH_NETBUS_OF_NETLIST(nl, mi, netbus)
	...
	//     printf("Number of ports: %u", ports->Size());
	//     printf("\n");
	//     const Port *my_port = (Port *)ports->GetAt(0);
	//     printf("port name: %s", port->Name());
	//     printf("\n");
	//     if (my_port->GetDir() == DIR_IN) {
	//       printf("input port\n");
	//     }
	//     if (my_port->GetDir() == DIR_OUT) {
	//       printf("output port\n");
	//     }
	//     if (my_port->GetDir() == DIR_INOUT) {
	//       printf("inoutput port\n");
	//     }
	//   }
	// }
